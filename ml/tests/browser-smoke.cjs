...
(async () => {
  ...
  try {
    ...
    assert.equal(closest.kind, 'closest_reference');
    assert.ok(closest.recipe_data);
    assert.match(closest.message, /closest/i);
    ...
    console.log('PASS: photo OCR → catalog recipe with no confirmation UI; out-of-catalog → closest reference; timer; invalid upload; same-origin; mobile layout; no JS errors.');
  } finally {
    await browser.close();
  }
